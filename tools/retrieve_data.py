...
def get_historical_price(crypto_name="ETH",
                         start=datetime.now() - relativedelta(days=7),
                         end=datetime.now(),
                         currency="JPY",
                         cache_path="cache"):
    limit = (end - start).days * 24 * 60
    ...
    while limit > 0:
        ...
        tmp = cryptocompare.get_historical_price_minute(coin=crypto_name, currency=currency, limit=amount, toTs=end)
        end -= timedelta(minutes=amount)
        limit -= amount
        values.extend(tmp)
